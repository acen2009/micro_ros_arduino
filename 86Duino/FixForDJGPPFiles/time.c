...
unsigned long long _86rdtsc(void)
{
        ...
        void)
{
    ...
    /* DJGPP/FreeDOS: Use _rdtsc() to convert nanoseconds, CLOCKS_PER_MICROSEC is obtained from 86Duino wiring.cpp */
    return (int64_t)(_86rdtsc() / CLOCKS_PER_MICROSEC) * 1000LL;
#else
    struct timespec ts;
    clock_gettime(CLOCK_REALTIME, &ts);
    return (((int64_t)ts.tv_sec) * 1000000000) + ts.tv_nsec;
